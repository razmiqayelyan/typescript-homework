...
interface OrganizedPostsType {
    byId: { [id: string]: PostType };
    allIds: string[];
}

const normalizeData = (unnormalizedData: PostType[]): OrganizedPostsType => {
    return unnormalizedData.reduce(
        ...
        { byId: {}, allIds: [] } as OrganizedPostsType
    );
};

const normalizedPosts: OrganizedPostsType = normalizeData(posts);
